...
public class DefaultAmazonSQSManager implements AmazonSQSManager {
    ...
    private LoadingCache<String, String> sqsQueueUrlsByName = CacheBuilder.newBuilder()
            .maximumSize(1000)
    ...
    private LoadingCache<String, String> sqsSinkQueueUrlsById = CacheBuilder.newBuilder()
            .maximumSize(1000)
            .build(new CacheLoader<String, String>() {
    ...
    @Override
    public String getSinkQueueUrlAndCreateIfNecessary(String moduleId) {
        try {
            return sqsSinkQueueUrlsById.get(moduleId);
        } catch (ExecutionException e) {
            // TODO: Better log message
            LOG.error("Failed to get queue url", e);
            return null;
        }
    }

    @Override
    public String getRpcRequestQueueNameAndCreateIfNecessary(String moduleId, String location) {
        final String rpcRequestQueueName = getRpcRequestQueueName(moduleId, location);
        try {
            sqsQueueUrlsByName.get(rpcRequestQueueName);
        } catch (ExecutionException e) {
            // TODO: Better log message
            LOG.error("Failed to get queue url", e);
        }
    ...
    @Override
    public String getRpcReplyQueueNameAndCreateIfNecessary(String moduleId, String location) {
        final String rpcReplyQueueName = getRpcReplyQueueName(moduleId, location);
        try {
            sqsQueueUrlsByName.get(rpcReplyQueueName);
        } catch (ExecutionException e) {
            // TODO: Better log message
            LOG.error("Failed to get queue url", e);
        }
    ...
    protected String getSinkQueueName(String moduleId) {
        ...
        return String.format("%s%s-%s%s", prefix, AmazonSQSConstants.AWS_SINK_QUEUE_PREFIX, moduleId, suffix);
    }

    private String ensureRpcQueueExists(String queueName) {
        return ensureQueueExists(queueName, Collections.emptyMap());
    }
